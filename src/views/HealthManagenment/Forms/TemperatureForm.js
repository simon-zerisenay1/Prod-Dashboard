...
import TemperatureImport from './TemperatureImport'
import DataManualAdd from './TemperatureAdd'
  ...
  return (
    <CRow>
      ...
      <div className="mb-4 col-md-12" style={{ display: isImport && !isAdd ? '' : 'none' }}>
        <TemperatureImport setisImport={setisImport} FilterAnimalsList={FilterAnimalsList} />
      </div>
      <CCol xs={12} style={{ display: isImport ? 'none' : '' }}>
        <h4 className="mb-3 mt-3">Temperatures List</h4>
      </CCol>
      ...
      <CCol lg={5} className="mb-lg-4 mb-4" style={{ display: isImport ? 'none' : '' }}>
        ...
        <CButton
          ...
          onClick={() => {
            setformID(0)
            setAutoID(0)
            document.getElementById('attachMentTF').value = ''
            setisisAdd(true)
            setisImport(true)
            ...
            AllAnimalList &&
              ...
              AllAnimalList.map((s, i) => {
                temp[i].status = 1
                if (document.getElementById(`tf_ani_${s.ani_id}`)) {
                  document.getElementById(`tf_ani_${s.ani_id}`).checked = false
                }
                return s
              })
            ...
            <CTable hover bordered responsive ref={tableRef}>
              <CTableHead>
                <CTableRow>
                  ...
                  <CTableHeaderCell scope="col">Temparature</CTableHeaderCell>
                  <CTableHeaderCell scope="col">Tool</CTableHeaderCell>
                      ...
                      <CIcon
                        ...
                        onClick={() => {
                          ...
                          if (confirm('Are you sure you want to Delete !') === true) {
                            deleteAnimalHealthData(singi.formID, 'temperatures')
                          }
                        }}
                      />
                    </CTableDataCell>
                    <CTableDataCell>{singi.animalID}</CTableDataCell>
                    <CTableDataCell>{singi.period}</CTableDataCell>
                    <CTableDataCell>
                      {singi.temparature} {singi.temparatureIn}
                    </CTableDataCell>
                    <CTableDataCell>{singi.tool}</CTableDataCell>
