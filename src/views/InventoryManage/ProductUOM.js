...
import ProductUOMCreate from './ProductUOMCreate'
...
const DepartmentsListPage = () => {
  ...
  const updateDataStatus = async (DataID, new_status) => {
    try {
      const res = await fetch(`${NODEAPIURL}/admin/product/updatePrdUOMStatus`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          status: new_status,
  ...
  return (
    <CRow>
      ...
      <CCol xs={12} style={{ display: ShowAddData ? 'none' : '' }}>
        <h3 className="mb-4">Manage Product UOM</h3>
      </CCol>
            ...
            <CTable hover bordered responsive ref={tableRef}>
              ...
              <CTableBody>
                {WorkedListData.map((singi) => (
                  <CTableRow key={singi.unit_id}>
                    <CTableDataCell>{singi.unit_id}</CTableDataCell>
                      ...
                      <CNav variant="pills">
                        <CDropdown variant="nav-item">
                          ...
                          <CDropdownMenu>
                            ...
                            <CDropdownItem
                              onClick={(e) => {
                                updateDataStatus(singi.unit_id, singi.status === 1 ? 0 : 1)
                              }}
                            >
