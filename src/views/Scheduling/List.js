...
import UploadActivity from './form'
...
const WorkersActivityList = () => {
  const [ExcelFileName, setExcelFileName] = useState('All Scheduling List')
  ...
  const FetchAttendanceListData = async () => {
    // alert(1)
    try {
      const res = await fetch(`${NODEAPIURL}/admin/scheduling/getListData`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Basic ${localStorage.getItem('token')}`,
  ...
  return (
    <CRow>
      ...
      <CCol style={{ display: ShowUploadActivity ? 'none' : '' }}>
        <h3 className="mb-3 mt-3">All Scheduling</h3>
      </CCol>
      ...
      <CCol md={4} className="mb-lg-4 mb-4" style={{ display: ShowUploadActivity ? 'none' : '' }}>
        ...
        <CButton
          ...
          className={isReportView ? 'text-white me-1 d-none' : 'd-none'}
          onClick={() => {
            FilterAttendanceListData()
            ...
            <CTable hover bordered responsive>
              <CTableHead>
                <CTableRow>
                  ...
                  <CTableHeaderCell scope="col">Send to</CTableHeaderCell>
                </CTableRow>
              </CTableHead>
              <CTableBody>
                {AttendanceListData.map((singi, i) => (
                  ...
                  >
                    <CTableDataCell>{singi.sch_id}</CTableDataCell>
                    ...
                    <CTableDataCell style={{ maxWidth: '250px' }}>
                      ...
                      >
                        {singi.description.trim() === '' ? 'N/A' : singi.description}
                      </div>
                      <button
                        type="button"
                        style={{
